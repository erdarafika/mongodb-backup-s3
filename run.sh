...
echo "=> Backup started"
if mongodump --host ${MONGODB_HOST} --port ${MONGODB_PORT} -u ${MONGODB_USER} -p ${MONGODB_PASS} -d ${MONGODB_DB} --authenticationDatabase admin --archive=\${BACKUP_NAME} --gzip ${EXTRA_OPTS} && aws $AWS_ARGS s3 cp \${BACKUP_NAME} \${S3BACKUP} && aws $AWS_ARGS s3 cp \${S3BACKUP} \${S3LATEST} && rm \${BACKUP_NAME} ;then
...
if aws s3 cp \${S3RESTORE} \${RESTORE_ME} && mongorestore --host ${MONGODB_HOST} --port ${MONGODB_PORT} -u ${MONGODB_USER} -p ${MONGODB_PASS} -d ${MONGODB_DB} --authenticationDatabase admin --drop ${EXTRA_OPTS} --archive=\${RESTORE_ME} --gzip && rm \${RESTORE_ME}; then
